...
require_once 'client_service_request_confirmation_mail.php';
class Users{
	...
	public function __construct(){
		...
		$this->mailsys = new client_service_request_confirmation_mail;
	...
	public function customerRegister(){
		...
		if($this->usermodel->register($data)){
			...
			$this->mailsys->registrationMail($_SESSION['email'],$_SESSION['UserName'],$data['usersPwd']);
			// $this->createUserSession($data['usersFirstName']." ".$data['usersLastName']);
			?>
			<script type="text/javascript">
				alert ("Succesfully account created ,please activate your account from given link in your mail.");
			</script>
			<?php
			flash("register","Succesfully account created ,please activate your account from given link in your mail.");
...
public function login(){
        ...
        if($this->usermodel->findUserByEmailOrUsername($data['Email'], $data['Email'])){
            ...
            if($loggedInUser){
            	...
            	$_SESSION['email'] = $data['Email'];
                flash("login", "Please Activate your Account first from given link in your email.. or wait for administartion authentication if you are service provider");
               redirect("../controllers/Users.php?q=login");
            }
        }else{
	...
	public function firstlogin(){
        ...
        if($this->usermodel->findUserByEmailOrUsername($data['Email'], $data['Email'])){
            //User Found
            $loggedInUser = $this->usermodel->firstlogin($data['Email'], $data['usersPwd']);
            if($loggedInUser){
                //Create session
				$_SESSION['email'] = $data['Email'];
				$result = $this->usermodel->getuserid($data['Email']);
                ...
                flash("login", "Password Incorrect");
                // redirect("../views/Homepage.php");
                redirect("../views/Homepage.php?login=true1");
            }
    ...
    public function createUserSession($user){
    	...
    	redirect("../views/admin_user_management.php");
    }
    }

    public function logout(){
    	$email = $_SESSION['email'];
    	$result = $this->usermodel->changestatus($email);
    	if($result){
...
else
{
	switch($_GET['q']){
		...
		case 'login':
			...
			break;
		case 'firsttimeLogin':
			$init->firsttimeLogin($_GET['name'],$_GET['password']);
			break;
		case 'about' :
		...
		case 'cust_servicehistory':
			redirect("../views/customer_service_history.php");
			break;
		case 'block_service_provider':
			redirect("../views/block_service_provider.php");
			break;
