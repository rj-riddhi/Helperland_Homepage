  ...
  $spid = $_SESSION['userid'];
    ...
    if($result){
      foreach($result as $row){
        ...
        $result4 = $this->usermodel->getBlockUnblockSp($spid,$custid);
        if($result4 > 0){
        ...
        <div class="col-md-4 pt-1">
            <div class="card pt-5 ">
                ...
                <div class="card-body text-center">
                    ...
                    <a style="color:white;font-weight:bold" class="btn badge_cancel" id="'.$custid.'" onclick="block('.$spid.','.$custid.','.$name2.')">'.$blockUnblock.'</a>
                </div>
            </div>
        </div>
    ';
...
public function getSpRatings(){
 ...
 if($result){
    foreach($result as $row){
      ...
      $result4 = $this->usermodel->getSpRatings2($spid,$serviceid);
      // $rating = $row['Ratings'];
      $rating = $result4[0];
      $comment = $result4[1];
...
public function Admin(){
  $input = file_get_contents('php://input');
    ...
    if($this->usermodel->register($data)){
      ...
      // $this->mailsys2->registrationMail($_SESSION['email'],$_SESSION['FirstName'],$data['usersPwd']);
      require_once '../controllers/Users.php';
      $this->users->createUserSession($data['usersFirstName']." ".$data['usersLastName']);
      echo json_encode(array("insert"=>"success"));
      }
    else
    {
