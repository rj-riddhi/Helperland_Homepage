...
// Dashboard page functions
    ...
    $data = '<tr id="'.$serviceid.'" onclick="summary(this.id,'.$serviceid.','.$date2.','.$time2.','.$printendtime2.','.$servicehours.','.$payment2.','.$comments2.','.$pets.','.$address.','.$mobile.','.$email.','.$spid.')">
       <td>
       ...
       <td>
          <div class="row">
           <div class="col-auto" >
              <img  style="border:1px solid #ccc;border-radius:50%;width:5rem;height:5rem" src="images/AvatarImages/'.$avtar.'">
            </div>
       ...
       <td class="d-flex">
         ...
         <span class=" badge_cancel" onclick="cancelmodal(this.id)" id="cancel'.$serviceid.'">Cancel</span>
         </td>
     </tr>';
...
public function updateservice(){
  ...
  $spid = $decode['spid'];
  if($spid != 0){
	$checforupdate = $this->usermodel->checkspotherrequests($date,$time,$id,$spid);
	...
	if($checforupdate){
		echo json_encode(array("insert"=>"fail",'msg'=>"Another service request has been assigned to the service provider on ".$date." from ".$starttime." to ".$endTime.". Either choose another date or pick up a different time slot"));
	}
	else{
...
$targetresult = $this->usermodel->GetUser($spid);
    ...
    $ratings = $ratingresult;
      ...
      <td><span class="btn badge_rate1" onclick="ratemodal(this.id,'.$spname2.')" id="rate '.$serviceid.'">Rate SP</span></td>
    </tr>';
...
public function checkoldpassword(){
  ...
  {
    echo json_encode(array("insert"=>"fail","msg"=>"Your old password is incorrect!!!"));
  }
}
...
public function passwordupdate(){
  ...
  {
    echo json_encode(array("insert"=>"fail","msg"=>"Your old password is not correct"));
  }
...
public function getAddresses($result){
  ...
  $result = $this->usermodel->deleteUserAddress($data);
  if($result){
    echo json_encode(array("insert"=>"success","msg"=>"Your Selected address is deleted successfully!!!"));
  }
  else{
    echo json_encode(array("insert"=>"fail","msg"=>"Your Selected address is not deleted please try again"));}
  }

  public function getBlockServiceProviders(){
  $custid = $_SESSION['userid'];
    $result = $this->usermodel->getBlockServiceProviders($custid);
    if($result){
      foreach($result as $row){
        $result2 = $this->usermodel->GetUser($row['ServiceProviderId']);
        $spid = $row['ServiceProviderId'];
        $name = $result2['FirstName'].' '.$result2['LastName'];
        $name2 = "'".$name."'";
        $avtar = $result2['UserProfilePicture'];
        $result3 = $this->usermodel->getFavUnfavouriteSp($custid,$spid);
        if($result3 > 0){
        $favUnfav = "Unfavourite";
      }
      else{
        $favUnfav = "Favourite";
      }
        $result4 = $this->usermodel->getBlockUnblockSp($custid,$spid);
        if($result4 > 0){
        ...
        <div class="col-md-4 pt-1">
            <div class="card pt-5 ">
                ...
                <div class="card-body text-center">
                    ...
                    <a style="color:white;font-weight:bold" class="btn badge_accept '.$spid.'" onclick="favourite('.$custid.','.$spid.','.$name2.')">'.$favUnfav.'</a>
                    <a style="color:white;font-weight:bold" class="btn badge_cancel '.$spid.'" onclick="block('.$custid.','.$spid.','.$name2.')">'.$blockUnblock.'</a>
                </div>
            </div>
        </div>
    ';
...
public function blockServiceProvider(){
  $input = file_get_contents('php://input');
  $decode = json_decode($input,true);
  $userid = $decode['userid'];
  $targetuserid = $decode['targetuserid'];
  $username = $decode['username'];
  $result = $this->usermodel->blockServiceProvider($userid,$targetuserid);
  if($result){
    echo json_encode(array("insert"=>"success","msg"=>"You successfully Blocked ".$username));
  }
  else{
    echo json_encode(array("insert"=>"fail","msg"=>"You are not able to block ".$username." remove this from fevorite service provider list."));
  }
}
public function unBlockServiceProvider(){
  $input = file_get_contents('php://input');
  $decode = json_decode($input,true);
  $userid = $decode['userid'];
  $targetuserid = $decode['targetuserid'];
  $username = $decode['username'];
  $result = $this->usermodel->unBlockServiceProvider($userid,$targetuserid);
  if($result){
...
public function FavouriteServiceProvider(){
  ...
  $targetuserid = $decode['targetuserid'];
  $username = $decode['username'];
  $result = $this->usermodel->favouriteServiceProvider($userid,$targetuserid);
  if($result){
    echo json_encode(array("insert"=>"success","msg"=>" ".$username." is added to your favourite Service provider's list"));
  }
  else{
    echo json_encode(array("insert"=>"fail","msg"=>"Please first unblock  ".$username));
  }
}
public function unFavouriteServiceProvider(){
  $input = file_get_contents('php://input');
  $decode = json_decode($input,true);
  $userid = $decode['userid'];
  $targetuserid = $decode['targetuserid'];
  $username = $decode['username'];
  $result = $this->usermodel->unFavouriteServiceProvider($userid,$targetuserid);
  if($result){
    echo json_encode(array("insert"=>"success","msg"=>"Now ".$username." is not in your favourite serviceprovider list"));
  }
  else{
    echo json_encode(array("insert"=>"fail","msg"=>"You are not able to remove ".$username." from your favouriteServiceProvider list."));
  }
}
...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
    ...
    case 'deleteaddress':
      $init->deleteaddress();
      break;
    case 'blockServiceProvider':
      $init->blockServiceProvider();
      break;
    case 'unBlockServiceProvider':
      $init->unBlockServiceProvider();
      break;
    case 'FavouriteServiceProvider':
      $init->FavouriteServiceProvider();
      break;
    case 'unFavouriteServiceProvider':
      $init->unFavouriteServiceProvider();
      break;
    case 'checkoldpassword':
      $init->checkoldpassword();
      break;  
    case 'passwordupdate':
      $init->passwordupdate();
      break;
    ...
    case 'getBlockServiceProviders':
      $init->getBlockServiceProviders();
      break;
